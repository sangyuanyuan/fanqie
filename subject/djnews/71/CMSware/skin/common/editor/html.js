...
var action=new Array("bold","italic","underline","/","justifyleft","justifycenter","justifyright","","insertimage","insertMultiImage","inserttable","inserthorizontalrule","createlink","","delete","cut","copy","paste","pasteText","pasteWord","undo","redo","","insertorderedlist","insertunorderedlist","outdent","indent","insertpage", "insertMedia", "wordclean","bb2html","editimage","insertattach","button_flash" )
var tooltip=new Array("粗体","斜体","下划线","/","向左对齐","向中靠","向右对齐","","单图插入","批量上传图片","插入表格","插入水平线","插入链接","","删除","剪切","复制","常规粘贴","粘贴纯文本","从Word中粘贴","撤消","恢复","","有序列表","无序列表","取消缩进","缩进","插入分页符", "多媒体插入", "Word格式清理", "bbcode转换html","图像处理","附件插入","Flash插入")
...
for (var i=0;i<action.length;i++) {
  if (action[i]=="/") s+='<img width="16" height="16" src="editor/images/forecolor.gif" class="Utoolbutton" onmouseover="this.className=\'Dtoolbutton\';" onmouseout="this.className=\'Utoolbutton\';" onclick="this.className=\'Ctoolbutton\';forecolor();" title="字体颜色" hspace="2" vspace="0">'
    ...
    s+='<img width="16" height="16" src="../html/editor/' + action[i] + '.gif" class="Utoolbutton" onmouseover="this.className=\'Dtoolbutton\';" onmouseout="this.className=\'Utoolbutton\';" onclick="this.className=\'Ctoolbutton\';'   
    if (i>4) { 
     if(i==8) s+='inSinglephoto();'
	 else if(i==9) s+='inMultiphoto();'
     else if(i==10) s+='intable();'
   else if(i==27) s+='insertpagesign();'
 	else if(i==28) s+='commonInputPicker(\'media\', \'\', \'\', \'parent.insertmedia\');'
 	else if(i==29) s+='Word_Clean(1,1);'
	else if(i==30) s+='bb2html();'
	else if(i==31) s+='imageEdit();'
	else if(i==32) s+='commonInputPicker(\'attach\', \'\', \'\', \'parent.insertattach\');'
	else if(i==33) s+='commonInputPicker(\'flash\', \'\', \'\', \'parent.insertflash\');'
	else if(i==18) s+='pasteText();'
	else if(i==19) s+='pasteWord();'
...
'     </td></tr></table><div id="HTMLEditor_Temp_HTML" style="VISIBILITY: hidden; OVERFLOW: hidden; POSITION: absolute; WIDTH: 1px; HEIGHT: 1px"></div>' ;
