...
{	
	var fitXP = "<?php echo $this->_tpl_vars["SYS_ENV"]["DialogFitXP"];?>";
	if(fitXP == "1") {
		//dialogWidth:300px;dialogHeight:266px;help:0;status:0;scroll:no
		re = /(dialogHeight:([0-9]*)px;)/i; 
		found = param.match(re); 
		if(found==null) {
			re = /(dialogHeight:([0-9]*)pt;)/i; 
			found = param.match(re); 
			myNum = new Number(found[2]);
			var newdialogHeight = myNum + 40;
			param = param.replace(found[1], "dialogHeight:" + newdialogHeight + "pt;"); 
		} else{
			myNum = new Number(found[2]);
			var newdialogHeight = myNum + 40;
			param = param.replace(found[1], "dialogHeight:" + newdialogHeight + "px;"); 
...
function commonDisplayPickerUI(action, srcValue) {
	...
	switch(action) {
		...
		case 'date':
			...
			break;
		case 'upload_img':
			var arr = showMeDialog('upload.php?sId=<?php echo $this->_tpl_vars["sId"];?>&o=display&mode=one&type=img_picker&NodeID=' + NodeID,"color","dialogWidth:390px;dialogHeight:120px;help:0;status:0;scroll:no");
			break;
		case 'upload_attach':
			var ab = showMeDialog('upload.php?sId=<?php echo $this->_tpl_vars["sId"];?>&o=display&mode=one&type=attach&NodeID=' + NodeID,"color","dialogWidth:390px;dialogHeight:120px;help:0;status:0;scroll:0");
			...
			break;
		case 'upload_flash':
			var arr = showMeDialog('upload.php?sId=<?php echo $this->_tpl_vars["sId"];?>&o=display&type=flash&NodeID=' + NodeID,"color","dialogWidth:390px;dialogHeight:120px;help:0;status:0;scroll:no");
			break;
		case 'tpl':
			var arr = showMeDialog("admin_select.php?sId=<?php echo $this->_tpl_vars["sId"];?>&o=tpl&tpl=" + srcValue,"color","dialogWidth:428px;dialogHeight:266px;help:0;status:0;scroll:no");
			break;	
		case 'psn':
			 
			
			var info = showMeDialog("admin_select.php?sId=<?php echo $this->_tpl_vars["sId"];?>&o=psn_picker&psn=" + srcValue ,"color","dialogWidth:600px;dialogHeight:266px;help:0;status:0;scroll:no");
...
----------------------------------- */
function commonInputPicker(action, sform, element, returnFunction)
{
	...
	var IMG_HTMLContent =  '<DIV>' + 
					'<table border="0" cellspacing="0" cellpadding="0" width=100% height="100%" style="BORDER-LEFT: buttonshadow 1px solid; BORDER-RIGHT: buttonshadow 1px solid; BORDER-TOP: buttonshadow 1px solid; BORDER-BOTTOM: buttonshadow 1px solid;" bgcolor=threedface> ' +
	...
	switch(action) {
		case 'img':
			oPopBody.innerHTML = IMG_HTMLContent;
 			oPopup.show(lefter - leftoff - 1 , topper - topoff + 18, 100, 90, document.body);
			break;
		case 'attach':
			oPopBody.innerHTML = ATTACH_HTMLContent;
 			oPopup.show(lefter - leftoff - 1 , topper - topoff + 18, 100, 90, document.body);
			break;
		case 'flash':
			oPopBody.innerHTML = FLASH_HTMLContent;
 			oPopup.show(lefter - leftoff - 1 , topper - topoff + 18, 100, 90, document.body);
			break;
		case 'media':
			oPopBody.innerHTML = MEDIA_HTMLContent;
 			oPopup.show(lefter - leftoff - 1 , topper - topoff + 18, 100, 70, document.body);
			break;
	}
...
{	
	...
	var info = showMeDialog("admin_resource.php?sId=<?php echo $this->_tpl_vars["sId"];?>&o=list_ui_main&Category=" + Category,"color","dialogWidth:563px;dialogHeight:412px;help:0;status:0;scroll:no");
