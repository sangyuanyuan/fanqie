<?
  ...
  $db = get_db();
  $newslist = $db->paginate('select n.id,n.title,n.last_edited_at,c.name as cname from smg_news n inner join smg_subject_items i on i.resource_id=n.id and i.category_type="news" and n.is_adopt=1 inner join smg_subject_category c on c.id=i.category_id and c.name="三分钟论坛" order by n.priority asc, n.last_edited_at desc',10,'threeminbbs');
  $newslist1 = $db->paginate('select n.id,n.title,n.last_edited_at,c.name as cname from smg_news n inner join smg_subject_items i on i.resource_id=n.id and i.category_type="news" and n.is_adopt=1 inner join smg_subject_category c on c.id=i.category_id and c.name="我为集团献一计" order by n.priority asc, n.last_edited_at desc',10,'ideaforsmg');  
?>
