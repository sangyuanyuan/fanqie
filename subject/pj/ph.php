...
<?
  ...
  $sql2=' and (created_at >="2009-01-01 00:00:00" and created_at<="2009-03-31 23:59:59" )';	
	...
	if($id==1)
	{
		...
		$strsql="select a.*,sum(countnum) as num from (select d.name,sum(n.click_count) as countnum from smg_dept d left join smg_news n on d.id=n.dept_id and d.id<>47 and is_recommend=1 ".$sql11." group by n.dept_id union select d.name,sum(n.click_count) as countnum from smg_dept d left join smg_video n on d.id=n.dept_id and d.id<>47 and is_recommend=1 ".$sql12." group by n.dept_id) as a group by name order by num desc";
