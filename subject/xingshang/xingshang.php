                  ...
                  <TABLE width="100%" border=0>
                    ...
                    <TR>
                      <TD><div class=title1>星尚互动</div></TD></TR>
                    	<TR>
                    		<? $newslist = $db->query('select * from smg_comment where resource_type="xingshang" order by createtime desc');?>
     ...
     <TR>
          <TD>
   		...
   		<? $news=$db->query('select n.id,n.description,n.title,n.news_type,n.target_url,n.file_name,c.id as cid from smg_news n inner join smg_subject_items i on i.resource_id=n.id and i.category_type="news" and n.is_adopt=1 inner join smg_subject_category c on c.id=i.category_id and c.name="我们的星尚" inner join smg_subject s on c.subject_id=s.id and s.name="星尚专题" order by n.priority asc, n.last_edited_at desc limit 6');
   			$photo=$db->query('select * from smg_photo where isadopt=1 and main_cate_id=40 order by priority asc,createtime desc',1,8);
   		?>
     <TR>
          <TD height=226 class=index_title>我们的星尚<a style="color:#F2E275;" target="_blank" style="margin-left:280px;" href="xslist.php?id=105">更多</a></TD></TR>
            <TABLE width="100%" border=0>
              <TBODY>
              	<tr>
              		<td colspan=4></td>
              	...
              	<tr>
              		<? for($i=0; $i< 4;$i++){?>
              			<td width="25%" height=100><a target="_blank" href="xslist.php?id=105"><img border=0 width=120 height=100 src="<? echo $photo[$i]->photourl;?>"></a></td>
              		<? }?>
              	</tr>
              	<tr>
              		<? for($i=4; $i< 8;$i++){?>
              ...
              <TR>
               ...
               <? $photo=$sqlmanager->GetRecords('select * from smg_photo where isadopt=1 and main_cate_id=38 order by priority asc,createtime desc',1,8)?>
          	...
          	<div id=demo3 style="overflow:hidden;height:129px;width:770px;">
				      	<table cellpadding="0" cellspacing="0" border="0">
									...
									<? for($i=0;$i<count($photo);$i++){?>
										<td><a target="_blank" href="http://www.enjoyoung.cn/ecms/enjoyoung/"><img border=0 height="129" src="<? echo $photo[$i]->photourl;?>" ></a></td>
									<? }?>
									</tr>
									</table>
									</td>
