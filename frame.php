<?php	
	...
	function show_content($table_name='smg_news',$type='news',$dept_name='',$category_name='',$limit=''){
		...
		if($table_name=='smg_link'){
			...
			$sql = 'select t1.dept_category_id,t1.title,t1.short_title,t1.id,t1.photo_src,t1.created_at,t1.description from '.$table_name.' t1 left join smg_category_dept t2 on t1.dept_category_id=t2.id left join smg_dept t3 on t2.dept_id=t3.id where t1.is_dept_adopt=1 and t2.name="'.$category_name.'" and t2.category_type="'.$type.'" and t3.name="'.$dept_name.'" order by t1.dept_priority,t1.created_at desc';
		}else{
