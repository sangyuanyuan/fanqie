<?php
	...
	$id = $_REQUEST['id'];
	$db = get_db();
	$sql = 'select description from smg_news where id='.$id;
	$record = $db->query($sql);
?>

<form action="flower.post.php" method="post" id='flower_from'>
<div style="width:280px; height:280px;  float:left; display:inline;">
<div style='width:280px; line-height:15px; float:left; display:inline;'><?php if($record[0]->description!=''){?>领导祝福：<?php echo strip_tags($record[0]->description);?><?php }?></div>
