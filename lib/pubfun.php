...
function search_keywords($id,$key,$table_name='smg_news',$about='',$page_count = 10, $order=''){
	...
	$d = implode(' and ',$d);
	$c = implode(' OR ' ,$c);
	$sql = "select n.title,n.id,n.created_at,n.category_id,c.platform from " .$table_name ." n inner join smg_category c on n.category_id=c.id and ".$d." and (".$c.")";
	if ($order){
		$sql = $sql . ' order  by ' .$order;
	}
	$db = get_db();
...
function search_newsid($id,$key,$table_name='smg_news',$page_count = 10, $order=''){
	...
	$sql = "select n.title,n.id,n.created_at,n.category_id,c.platform from " . $table_name ." n inner join smg_category c on n.category_id=c.id and n.is_adopt=1 and n.id<>".$id." and n.id in (".$key.")";
	if ($order){
		$sql = $sql . ' order  by ' .$order;
	}
	$db = get_db();
