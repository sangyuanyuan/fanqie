...
function search_keywords($key,$table_name='smg_news',$about='',$page_count = 10, $order=''){
	...
	$sql = 'select n.*,c.platform from ' . $table_name ." n inner join smg_category c on n.category_id=c.id and 1=1 and " .$d." and (".$c .")";
	if ($order){
		$sql = $sql . ' order  by ' .$order;
	}
...
function search_newsid($key,$table_name='smg_news',$page_count = 10, $order=''){
	...
	$sql = 'select n.* from ' . $table_name ." n inner join smg_category c on n.category_id=c.id  and n.id in (".$key.")";
	if ($order){
		$sql = $sql . ' order  by ' .$order;
	}
	$db = get_db();
