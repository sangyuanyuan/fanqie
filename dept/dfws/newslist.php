...
<body>
	...
	<div id=east_body>
		...
		<span style="float:left; display:inline">滚动新闻：</span>
		...
		<div id=east_content>
			...
			<div id=context>
				...
				<?
				 ...
				 ?>
					<div id=title><? echo $newslist->categoryname;?></div>
					<div id=content>
						<div id=newslisttop>
							<div id=left>
							<? for($i=0;$i<7 && $i<$newslist->itemcount;$i++){?>
								<a href="/dfws/news.php?id=<? echo $newslist->items[$i]->id; ?>">
									·<? echo $newslist->items[$i]->title;?>
								</a><? }?>
								</div>
							<div id=right>
							<? for($i=7;$i<14 && $i<$newslist->itemcount;$i++){?>
								<a href="/dfws/news.php?id=<? echo $newslist->items[$i]->id; ?>">
									· <? echo $newslist->items[$i]->title;?>
									</a><? }?></div>
						</div>
						<div id=newslistbottom><? echo $newslist->showpages(); ?></div>
					</div>
				</div>
				<div id=news_right>	
					<div id=top>
